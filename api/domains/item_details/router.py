...
@router.post("/", response_model=schemas.ItemDetailRead, status_code=status.HTTP_201_CREATED)
def create_item_detail(detail: schemas.ItemDetailCreate, db: Session = Depends(get_db)):
    return service.create_item_detail(db, detail)


@router.get("/", response_model=List[schemas.ItemDetailRead])
...
@router.get("/{detail_id}", response_model=schemas.ItemDetailRead)
...
@router.patch("/{detail_id}", response_model=schemas.ItemDetailRead)
def update_item_detail(detail_id: int, detail_update: schemas.ItemDetailUpdate, db: Session = Depends(get_db)):
...
@router.delete("/{detail_id}", response_model=schemas.ItemDetailRead)
